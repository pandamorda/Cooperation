...
import {
    PROGRESSBAR_PRECISION,
    PAGE_URLS,
    REDIRECT_TIME
} from '@constants/_common';
...
const updateSolvedIdsArray = (
    questionId = '',
    questionType = '',
    solvedQuestionsArray = [],
    questionTypes = {}
) => {
    const inputElemsArray = Array.from(getElsByName(questionId));
    ...
    switch (questionType) {
        ...
        case questionTypes.code: {
            isSolved = inputElemsArray.every(
...
const updateProgressbar = (solvedQuestionsArray = [], questionsArray = []) => {
    const testProgress =
        (solvedQuestionsArray.length / questionsArray.length) * 100 || 0;
...
const updateNav = (questionId = '', solvedQuestionIdsArray = []) => {
    const questionAnchorElem = Array.from(
        getElById(ID.navList).getElementsByTagName('a')
    )
        .filter((anchor) => anchor.href.split('#').pop() === questionId)
        .pop();

    questionAnchorElem.textContent =
        solvedQuestionIdsArray.indexOf(questionId) !== -1
...
const generateInputs = (
    ...
    question.answers.forEach((answer, index) => {
        const inputElem = createEl('input');
        ...
        const labelElem = createEl('label');
        labelElem.setAttribute('for', inputElem.id);

        let inputWrapperElem = createEl('p');
        ...
        switch (question.type) {
            ...
            case questionTypes.multiImg: {
                ...
                inputWrapperElem = createEl('figure');
                break;
            }
            case questionTypes.match: {
                setInputEventlistener(
...
const generateQuestions = (
    ...
    const questionsNav = getElById(ID.navList);
    const buttonSubmit = getElById(ID.buttonSubmit);

    testData.questions.forEach((question, questionIndex) => {
        const questionId = `${TESTS.questionIdPrefix}${questionIndex}`;

        const inputsFieldsetElem = createEl('fieldset');
...
const submitForm = async (testData = {}, questionTypes = {}) => {
    ...
    testResultsData.questions = testResultsData.questions.map(
        (question, index) => {
...
const handleFormLoad = async () => {
    ...
    const handleFormSubmit = (event) => {
        ...
        Array.from(getElsByTag('input'))
            .concat(Array.from(getElsByTag('textarea')))
            .concat(getElById(ID.buttonSubmit))
    ...
    getElById(ID.testName).textContent = test.data.name;
