...
import { redirectPage } from '@scripts/_common';
...
export class Console extends ConsoleBase {
    _userCommands = {
        ...this._userCommands,
        card: new Command(
            'Перегляд особистої картки користувача',
            (userCard = this._userData.card) => {
                const cardEntries = Object.entries(userCard).map(
        ...
        commands: new Command('Інормація про команди користувача', () => {
            this._cout('\nДоступні команди:');
            Object.entries(this._userCommands).forEach((command) =>
                this._cout(` - ${command[0]} - ${command[1].description}`)
            );
        }),
        ...
        tests: new Command('Інормація про тестування', async () => {
            ...
            this._cout(
                `Розпочато '${requiredTests.at(testIndex)}'. Перенаправлення...`
            );
            await redirectPage(
                PAGE_URLS.tests,
                '_blank',
    ...
    constructor(config = {}, userData = {}) {
        ...
        document.getElementById(CONSOLE.elemIds.userName).innerText =
            this._userData.card.nickname;
        this._init();
    }
