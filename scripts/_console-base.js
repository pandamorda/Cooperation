...
class ConsoleBase {
    ...
    _htmlElem;
    _userCommands = {
        clear: new Command('Очищення екрану терміналу', async () => this._htmlElem.value = ''),
    };
    ...
    constructor(config = {}, userData = {}) {
        ...
        this._htmlElem.addEventListener('focus', (event) => this.#handleFocus(event));
        this._htmlElem.addEventListener('keydown', (event) => this.#handleKeyDown(event));
    ...
    _dcout(delim = '\n', ...values) {
        values.forEach(val => {
            this._htmlElem.value = this._htmlElem.value.concat(`${delim}${val.toString()}`);
    ...
    async _cin(validate = (_cinValue) => true, prompt = '', errorMsg = '') {
    ...
    #handleKeyDown(event) {
        ...
        switch (eventKeyCode) {
            ...
            case 13: {
                ...
                this.#buffer = this._htmlElem.value.slice(this.#staticTextLenght).trim() || '\n';
                this.#staticTextLenght = this._htmlElem.value.length;
                break;
            }
            default: {
    ...
    #logConsoleInit() {
        ...
        this._cout(
            `InitiativeGroup, site v${this._config.version} (v${this._config.versionPrecise}, ${new Date()}) [HTML-CSS & JS (spec.)] on linux`,
    ...
    async #runCommandHandler() {
        while (true) {
            await new Promise(async (fulfill) => {
                const userCommand = await this._cin((value) => this._userCommands.hasOwnProperty(value), null, "Incorrect user command");
    ...
    #setCursorPosition(position) { this._htmlElem.selectionStart = position };
}